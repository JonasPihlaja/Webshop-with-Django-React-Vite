...
def load_json_from_dist(json_filename="manifest.json"):
    manifest_file_path = Path(str(settings.PROJECT_ROOT), "frontend", "dist", ".vite", json_filename)
...
def render_vite_bundle():
    ...
    imports_files = "".join(
        [
            f'<script type="module" src="{settings.STATIC_URL}/{manifest[file]["file"]}"></script>'
            for file in files
            if manifest[file].get("file", "")
        ]
        + [
            f"""<link rel="stylesheet" type="text/css" href="{settings.STATIC_URL}/{css}" />"""
            for file in files
    ...
    return mark_safe(
        f"""<script type="module" src="{settings.STATIC_URL}/{manifest['index.html']['file']}"></script>
        <link rel="stylesheet" type="text/css" href="{settings.STATIC_URL}/{manifest['index.html']['css'][0]}" />
        {imports_files}"""
    )